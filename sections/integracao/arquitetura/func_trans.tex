...
Como abordado na seção \ref{fsm}, a dinâmica do sistema é descrita por uma função de transição determinística $\delta$, que mapeia o estado atual do sistema para seu estado sucessor a cada evento de clock.
...
\begin{align*}
(X_{u,t+1}, c_{int}) &= \delta_X^{(u)}(X_{u,t}, c_{in}, e_X) \\
...
\begin{itemize}
    ...
    \item $c_{in}$ é o carry/borrow de entrada;
...
Essa abordagem permite reduzir a complexidade da função de transição global, substituindo uma única função de alta dimensionalidade por um conjunto de funções locais de menor complexidade, interligadas por sinais de controle simples.